...
class EuclideanCodebook(nn.Module):
    ...
    def update_in_place(self) -> None:
        """Recompute the centroids and their squared norms from the weights."""
    ...
    def encode(self, xs: mx.array) -> mx.array:
        ...
        dot_product = xs @ self._embedding.swapaxes(-1, -2)
        return (self._c2 - dot_product).argmin(axis=-1).reshape(target_shape)
...
class ResidualVectorQuantization(nn.Module):
    ...
    def __init__(self, nq: int, dim: int, codebook_size: int, codebook_dim: int | None):
        ...
        self.layers = [
            VectorQuantization(dim=dim, codebook_size=codebook_size, codebook_dim=codebook_dim)
            for _ in range(nq)
        ]
    ...
    def decode(self, xs: mx.array) -> mx.array:
        ...
        for index in range(1, xs.shape[0]):
            quantized = quantized + self.layers[index].decode(xs[index])
        return quantized
...
class ResidualVectorQuantizer(nn.Module):
    def __init__(
        ...
        if input_dim != dim or force_projection:
            self.input_proj = Conv1d(input_dim, dim, 1, bias=False)
        self.output_proj = None
        if output_dim != dim or force_projection:
    ...
    def decode(self, xs: mx.array) -> mx.array:
        quantized = self.vq.decode(xs.swapaxes(0, 1))
...
class SplitResidualVectorQuantizer(nn.Module):
    ...
    def __init__(self, dim: int, input_dim: int | None, output_dim: int | None, nq: int, bins: int):
        super().__init__()
    ...
    def encode(self, xs: mx.array) -> mx.array:
        ...
        if self._nq > 1:
            codes = mx.concatenate([codes, self.rvq_rest.encode(xs)], axis=1)
        return codes
